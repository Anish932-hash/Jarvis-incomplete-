...
def test_desktop_machine_profile_builds_recommendations_and_persists_preferences(tmp_path) -> None:
    ...
    snapshot = manager.build_snapshot(
        system_profile={
            ...
            "dependencies": {
                ...
                "ocr_ready": False,
                "vision_ready": False,
                "python_packages": {"cython": False},
            },
            "permissions": {"cwd_writable": True, "temp_writable": True},
            "virtualization": {"virtualization_firmware_enabled": False},
            "runtimes": {
    ...
    assert snapshot["applications"]["inventory_count"] == 3
    assert snapshot["applications"]["remembered_target_count"] == 0
    assert snapshot["providers"]["summary"]["provider_count"] == 2
    assert snapshot["providers"]["summary"]["invalid_count"] >= 1
    assert snapshot["models"]["inventory_count"] == 1
    assert snapshot["readiness"]["overall_status"] in {"attention", "partial", "strong", "ready"}
    ...
    assert "install_native_build_toolchain" in recommendation_codes
    assert "install_ocr_runtime" in recommendation_codes
    assert "install_vision_runtime" in recommendation_codes
    assert "install_cython_runtime" in recommendation_codes
    assert "enable_virtualization_support" in recommendation_codes
    ...
    changed_snapshot = manager.build_snapshot(
        system_profile={
            "hostname": "jarvis-dev",
            "windows": {"caption": "Windows 11 Pro", "build_number": "22635"},
            "cpu": {"name": "AMD Ryzen 9", "logical_cores": 24},
            "gpus": [{"name": "RTX 4080", "adapter_ram_bytes": 16 * 1024 * 1024 * 1024}],
            "dependencies": {
                "ready_count": 6,
                ...
                "ocr_ready": True,
                "vision_ready": True,
                "python_packages": {"cython": True},
            },
            "permissions": {"cwd_writable": True, "temp_writable": True},
            "virtualization": {"virtualization_firmware_enabled": True},
            "python": {"version": "3.12.2"},
            "runtimes": {
                "rustc": {"available": True, "version": "rustc 1.89.0"},
                "cargo": {"available": True, "version": "cargo 1.89.0"},
                "cython": {"available": True, "version": "Cython version 3.1.0"},
                "tesseract": {"available": True, "version": "tesseract 5.4.0"},
                "huggingface_cli": {"available": True},
                "hf": {"available": True},
            },
        },
        app_inventory={
        ...
        launch_memory={
            ...
            "total": 2,
            "items": [
                {"kind": "path", "resolution": "launch_memory", "launch_count": 2, "resolve_count": 4},
                {"kind": "running_process", "resolution": "running_process", "launch_count": 1, "resolve_count": 1},
            ],
        },
        provider_snapshot={
            "providers": {
                "huggingface": {"present": True, "ready": True, "required_by_manifest": True},
                "groq": {"present": True, "ready": True, "required_by_manifest": False},
            },
        ...
        provider_verifications={
            "huggingface": {"status": "success", "verified": True, "summary": "Verified Hugging Face access"},
            "groq": {"status": "success", "verified": True, "summary": "Verified Groq access"},
        },
        local_models={
            "task_counts": {"reasoning": 1, "vision": 1},
            "missing_task_counts": {},
            "inventory": {
                "present_count": 2,
                "items": [
                    {"task": "reasoning", "present": True, "name": "qwen3-14b", "path": r"E:\models\qwen3-14b.gguf", "backend": "llama_cpp"},
                    {"task": "vision", "present": True, "name": "florence-2", "path": r"E:\models\florence-2.gguf", "backend": "llama_cpp"},
                ],
            },
            "bridge_profiles": [
                {"task": "reasoning", "provider": "local", "name": "qwen3-14b", "path": r"E:\models\qwen3-14b.gguf", "execution_backend": "llama_cpp", "launch_ready_count": 1}
            ],
        },
        model_setup_workspace={"status": "success", "recommendations": []},
        task_preferences=manager.task_model_preferences(),
        source="unit_test_recheck",
    )
    assert changed_snapshot["change_detection"]["changed"] is True
    assert "windows_build" in changed_snapshot["change_detection"]["areas"]
    assert changed_snapshot["readiness"]["revalidation_required"] is True
