...
class DesktopBenchmarkLabCampaignSupervisor:
    def __init__(
        self,
        *,
        state_path: str = "data/desktop_benchmark_campaign_supervisor.json",
        enabled: bool = False,
    ...
    def start(self, execute_callback: Callable[..., Dict[str, Any]]) -> None:
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._worker,
                name="desktop-benchmark-campaign-supervisor",
                daemon=True,
            )
            self._thread.start()
        ...
        *,
        ...
        with self._lock:
            ...
            if campaign_status is not None:
                self._config["campaign_status"] = str(campaign_status or "").strip()
            if pack is not None:
    ...
    def _execute_once(self, *, source: str, overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        with self._lock:
            if bool(self._runtime.get("inflight", False)):
                return {
                    "status": "busy",
                    "message": "benchmark campaign daemon tick already in progress",
            ...
            if callback is None:
                return {
                    "status": "unavailable",
                    "message": "benchmark campaign daemon callback unavailable",
                    "supervisor": self._public_status_locked(),
                }
        ...
        result_payload = (
            ...
            else {"status": "error", "message": "invalid benchmark campaign supervisor result"}
        )
        with self._lock:
            ...
            self._runtime["run_count"] = self._coerce_int(
                self._runtime.get("run_count", 0), minimum=0, maximum=1_000_000, default=0
            ) + 1
            if source == "daemon":
    ...
    def _public_status_locked(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "campaign_status": str(self._config.get("campaign_status", "") or "").strip(),
            ...
            "manual_trigger_count": self._coerce_int(
                self._runtime.get("manual_trigger_count", 0), minimum=0, maximum=1_000_000, default=0
            ),
            "auto_trigger_count": self._coerce_int(
                self._runtime.get("auto_trigger_count", 0), minimum=0, maximum=1_000_000, default=0
            ),
            "consecutive_error_count": self._coerce_int(
                self._runtime.get("consecutive_error_count", 0), minimum=0, maximum=1_000_000, default=0
            ),
    ...
    def _result_summary(result_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "targeted_campaign_count": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                payload.get("targeted_campaign_count", 0), minimum=0, maximum=100_000, default=0
            ),
            ...
            "pending_session_count": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                payload.get("pending_session_count", 0), minimum=0, maximum=100_000, default=0
            ),
            "attention_session_count": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                payload.get("attention_session_count", 0), minimum=0, maximum=100_000, default=0
            ),
            "pending_app_target_count": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                payload.get("pending_app_target_count", 0), minimum=0, maximum=100_000, default=0
            ),
            "long_horizon_pending_count": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                payload.get("long_horizon_pending_count", 0), minimum=0, maximum=100_000, default=0
            ),
            "error_count": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
    ...
    def _default_config(
        *,
        ...
        return {
            ...
            "interval_s": DesktopBenchmarkLabCampaignSupervisor._coerce_float(
                interval_s, minimum=5.0, maximum=3600.0, default=180.0
            ),
            ...
            "max_replays_per_session": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                max_replays_per_session, minimum=1, maximum=8, default=2
            ),
            "history_limit": DesktopBenchmarkLabCampaignSupervisor._coerce_int(
                history_limit, minimum=1, maximum=64, default=8
            ),
            "campaign_status": str(campaign_status or "").strip(),
